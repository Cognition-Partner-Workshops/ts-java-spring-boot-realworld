...
public class UsersApi {
  ...
  public ResponseEntity createUser(@Valid @RequestBody RegisterParam registerParam) {
    UserWithToken userWithToken = userFacade.registerUser(registerParam);
    return ResponseEntity.status(201).body(userResponse(userWithToken));
  }

  @RequestMapping(path = "/users/login", method = POST)
  public ResponseEntity userLogin(@Valid @RequestBody LoginParam loginParam) {
    UserWithToken userWithToken = userFacade.login(loginParam.getEmail(), loginParam.getPassword());
