...
public class CommentsApiTest extends TestWithCurrentUser {
  ...
  @MockBean private CommentFacade commentFacade;
  ...
  public void should_create_comment_success() throws Exception {
    ...
    when(commentFacade.createComment(eq(article.getSlug()), anyString(), any(User.class)))
  ...
  public void should_get_comments_of_article_success() throws Exception {
    when(commentFacade.getComments(eq(article.getSlug()), eq(null)))
  ...
  public void should_delete_comment_success() throws Exception {
    doNothing().when(commentFacade).deleteComment(eq(article.getSlug()), eq(comment.getId()), any(User.class));
  ...
  public void should_get_403_if_not_author_of_article_or_author_of_comment_when_delete_comment()
    ...
    doThrow(new NoAuthorizationException()).when(commentFacade)
        .deleteComment(eq(article.getSlug()), eq(comment.getId()), any(User.class));
