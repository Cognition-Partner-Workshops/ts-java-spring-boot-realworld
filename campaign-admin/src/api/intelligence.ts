...
export async function startResearch(
  ...
  const res = await fetch(INTEL_BASE, {
    method: 'POST',
...
export async function getResearch(id: string): Promise<EvidencePack> {
  const res = await fetch(`${INTEL_BASE}/${id}`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function getReport(id: string): Promise<string> {
  const res = await fetch(`${INTEL_BASE}/${id}/report`);
  if (!res.ok) throw new Error(await res.text());
  return res.text();
}
...
export async function deleteResearch(id: string): Promise<void> {
  const res = await fetch(`${INTEL_BASE}/${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error(await res.text());
}
